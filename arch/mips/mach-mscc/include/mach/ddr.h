...
{
	setbits_le32(REG_CFG(ICPU_MEMPHY_CFG), ICPU_MEMPHY_CFG_PHY_FIFO_RST);
	PAUSE();
	clrbits_le32(REG_CFG(ICPU_MEMPHY_CFG), ICPU_MEMPHY_CFG_PHY_FIFO_RST);
	PAUSE();
}
...
static inline void hal_vcoreiii_ddr_reset_assert(void)
{
	...
	setbits_le32(REG_GCB(PERF_GPIO_OE), BIT(19));
...
static inline void hal_vcoreiii_ddr_verified(void)
{
	...
	/* Finally, enable ECC */
	register u32 val = readl(REG_CFG(ICPU_MEMCTRL_CFG));
...
static inline void hal_vcoreiii_init_memctl(void)
{
	...
	clrbits_le32(REG_CFG(ICPU_RESET), ICPU_RESET_MEM_RST_FORCE);
...
static inline void hal_vcoreiii_wait_memctl(void)
{
	...
	while (!(readl(REG_CFG(ICPU_MEMCTRL_STAT))
